...
from epc_modules.utils.quality_gate import QualityTemplateCloner, NCRManager
from epc_modules.utils.constants import (
    BILLING_TRACKS, WBS_ARCHITECTURES, INVENTORY_STRATEGIES
)
...
def add_project_role(doc):
    ...
    for role in roles_to_add:
        try:
            ...
            logger.info(f"Project {doc.name} visibility granted to role: {role}")
        except Exception as e:
            logger.warning(f"Could not add role {role} for project {doc.name}: {e}")
...
def on_project_created(doc, method):
    ...
    logger.info(f"Initializing EPC project: {doc.name}")
    ...
    try:
        ...
        if typology.requires_itp:
            ...
            logger.info(f"Inspection templates cloned for project {doc.name}")

        # Add project to correct role visibility
        add_project_role(doc)

        frappe.db.commit()
        logger.info(f"EPC project {doc.name} initialized with typology: {typology.name}")

    except Exception as e:
        logger.error(f"Failed to initialize EPC project {doc.name}: {str(e)}")
        raise
...
def on_project_updated(doc, method):
    ...
    if doc.is_epc_project and doc.project_typology:
        logger.info(f"Project updated: {doc.name}")
...
def validate_civil_requirements(doc, typology):
    ...
    if not doc.contract_value:
        logger.warning(f"Civil project {doc.name} has no contract value set")
...
def on_po_validate(doc, method):
    ...
    logger.info(f"Validating PO {doc.name} for EPC project {doc.project}")
...
def validate_tbe_for_electromechanical(doc, method):
    ...
    if typology.typology_type == "Electromechanical" and typology.requires_tbe:
        ...
        logger.info(f"TBE validation for PO {doc.name} in electromechanical project")
...
def validate_site_zone_allocation(doc, method):
    ...
    if typology.requires_spatial_zones:
        ...
        logger.info(f"Zone validation for Purchase Receipt {doc.name}")
...
def validate_stock_entry_project(doc, method):
    ...
    logger.info(f"Stock Entry {doc.name} for EPC project {doc.project}")
...
def validate_ra_bill_integration(doc, method):
    ...
    logger.info(f"Sales Invoice {doc.name} for EPC project {doc.project}")
...
def after_install():
    ...
    """
    logger.info("Running EPC module post-installation setup")

    try:
        # Set up all workflows
        setup_epc_workflows()
        logger.info("EPC workflows configured successfully")

        # Create default typologies if needed
        create_default_typologies()

        logger.info("EPC module post-installation complete")
    except Exception as e:
        logger.error(f"Post-installation setup failed: {str(e)}")
        raise
    ...
    for typology_config in default_typologies:
        if not frappe.db.exists("Project Typology", typology_config["name"]):
            try:
                ...
                logger.info(f"Created default typology: {typology_config['name']}")
            except Exception as e:
                logger.warning(f"Could not create typology {typology_config['name']}: {e}")
...
def on_ncr_status_change(doc, method):
    ...
    if doc.status == "Closed":
        ...
        logger.info(f"NCR {doc.ncr_number} closed for project {doc.project}")
...
def on_cube_test_submit(doc, method):
    ...
    if doc.age_days == 28 and not doc.is_pass:
        ...
        logger.warning(f"Failed cube test {doc.cube_test_id} may trigger NCR")
...
def create_concrete_ncr(cube_doc):
    ...
    try:
        ...
        logger.info(f"Created NCR for failed cube test {cube_doc.cube_test_id}")
    except Exception as e:
        logger.error(f"Failed to create NCR for cube test {cube_doc.name}: {e}")
...
def on_mix_design_approval(doc, method):
    ...
    if doc.approval_status == "Approved":
        ...
        logger.info(f"Mix design {doc.mix_design_code} approved")
...
def send_dashboard_alerts():
    ...
    if critical_alerts:
        ...
        logger.warning(f"Found {len(critical_alerts)} critical EPC alerts")
...
def on_risk_materialized(doc, method):
    ...
    if doc.status == "Materialized":
        ...
        logger.warning(f"Risk materialized: {doc.risk_id} - {doc.risk_title}")
