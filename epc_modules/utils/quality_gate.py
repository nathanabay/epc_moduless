...
class ITPManager:
    ...
    def _create_ncr_from_inspection_fail(record: "frappe.doc") -> None:
        ...
        NCRManager.create_ncr_from_inspection(
            ...
            # Always Critical — this function is only called when is_within_tolerance is False
            severity="Critical",
    ...
    def _update_itp_summary(itp_name: str) -> None:
        ...
        # Get all hold points from inspection records
        hold_points_with_mandatory = [
            frappe.get_doc("Inspection Hold Point", r.hold_point)
            for r in itp.inspection_records
        ]

        # All mandatory hold points (is_mandatory == 1) must have status "Pass"
        all_mandatory_passed = all(
            hp.is_mandatory != 1 or hp.status == "Pass"
